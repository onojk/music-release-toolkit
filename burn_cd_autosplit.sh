#!/usr/bin/env bash
# burn_cd_autosplit.sh — Audio CD DAO burner with auto-split, chatty progress, zero waits
# - Starts immediately (no countdowns)
# - Converts all inputs to 44.1 kHz / 16-bit / stereo WAV
# - Honors playlist.m3u (if present), else Number*.wav, else *.wav (numeric sort)
# - Auto-splits over multiple discs if needed (80-min by default)
# - Optional loudness normalize (EBU-ish) per track
# - Burns with wodim in DAO mode (-dao -pad), with BURN-Free
# - Shows conversion progress (ffmpeg -stats) and print-as-you-go disc plans
# - Caches sudo once up-front so burning can begin immediately
set -euo pipefail
...
OVERBURN=0                # --overburn (usually not needed since we split)
...
FORCE_CONVERT=0           # --force-convert (rebuild staged WAVs)
...
Options:
  ...
  --force-convert          Rebuild staged WAVs even if already present
...
auto_device() {
  # Prefer /dev/sr0 if present; else try to detect a CD/DVD device via lsblk
  ...
  # Try lsblk to find any "rom" device
  local cand
  cand="$(lsblk -o NAME,TYPE | awk '$2=="rom"{print $1; exit}' || true)"
  if [[ -n "$cand" && -e "/dev/$cand" ]]; then
    echo "/dev/$cand"; return
  fi
  echo "/dev/sr0"   # best guess
}

# Temporarily disable Ctrl+Z during critical burn step (prevent accidental suspend)
disable_ctrl_z() {
  STTY_SUSP_SAVE="$(stty -a | sed -n 's/.*susp = \([^;]*\);.*/\1/p' || true)"
  stty susp undef || true
}
restore_ctrl_z() {
  [[ -n "${STTY_SUSP_SAVE:-}" ]] && stty susp "$STTY_SUSP_SAVE" || true
}
...
for inwav in "${TRACKS[@]}"; do
  ...
  if [[ $FORCE_CONVERT -eq 1 || ! -f "$out" ]]; then
    ...
    log "  #$idx  already staged: $(basename "$out")"
  fi
...
make_disc_dir() { local n="$1"; local d; d="$(printf '_cd_disc%02d' "$n")"; mkdir -p "$d"; echo "$d"; }
...
for i in "${!STAGE_FILES[@]}"; do
  ...
  # If any single track exceeds capacity and no overburn -> fail (can't split a track automatically)
  if (( s > cap_sec )) && (( OVERBURN == 0 )); then
    die "Track $(basename "$f") is longer than ${CAPACITY_MINS} minutes. Split/trim the track or re-run with --overburn (not guaranteed)."
  fi
  ...
  # Hardlink to avoid duplicating data; fall back to copy if FS disallows
...
burn_one_disc() {
  ...
  # Prevent accidental Ctrl+Z during the actual burn
  disable_ctrl_z
...
if (( CLEANUP == 1 )); then
  ...
  rm -rf --one-file-system "$WORK_ALL" _cd_disc??
fi
